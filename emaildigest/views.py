...
from .models import (AnonymousSubscription, UnSubscription, Subscription,
                     EmailDigest)
...
def subscribe(request):
    ...
    if thankyou:
        return render(request,
                      'emaildigest/subscribe_thankyou_%s.html'%(thankyou),
                      {'prevent_footer_subscription_form': True})
    verification_code = request.GET.get('v', None)
    if verification_code:
        subscription = get_object_or_404(AnonymousSubscription,
        ...
        return render(request, 'emaildigest/subscribe_verification_done.html',
                      {'prevent_footer_subscription_form': True})
    ...
    return render(request, 'emaildigest/subscribe.html',
...
def unsubscribe(request, subscription_id=None, digest_id=None):
    if 'done' in request.GET.keys():
        ...
        return render(request, 'emaildigest/unsubscribe_done.html',
                      {'email': email,
    ...
    if subscription_id is None and digest_id is None:
        ...
        return render(request, 'emaildigest/unsubscribe.html',
