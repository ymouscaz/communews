...
from django.db.models import (Value, F, Func, ExpressionWrapper, fields, Q,
                              Min, OuterRef, Subquery)
from django.db.models.functions import Power, Now, Cast, Extract
...
def _front_page(paging_size=settings.PAGING_SIZE, page=0, add_filter={},
                add_q=[], as_of=None, days_back=50):
    ...
    if connection.vendor == 'postgresql':
        ...
        return Story.objects.select_related('user')\
                            ...
                            .filter(created_at__gte=now - datetime.timedelta(
                                                            days=days_back))\
...
@ratelimit(key="user_or_ip", group="news-get", rate=DEFAULT_GET_RATE,
           block=True)
def index(request):
    page = int(request.GET.get('p', 0))
    stories = cache.get_or_set("news-index-%s" % (page),
                               lambda: list(_front_page(page=page)),
                               timeout=TIMEOUT_MEDIUM)  # one minute
...
def show(request):
    ...
    stories = cache.get_or_set(
                        "news-show-%s" % (page),
                        lambda: list(_front_page(
                                            page=page,
                                            add_filter={'is_show': True}
                                                )
                                     ),
                        timeout=TIMEOUT_MEDIUM)  # one minute
    if len(stories) < 1 and page != 0:
...
def newest(request):
    ...
    if 'upvoted_by' in request.GET.keys():
        ...
        add_filter['pk__in'] = Vote.objects.filter(vote=1, user=request.user)\
                                           .values('item')
        add_q.append(~Q(user=request.user))
    if 'site' in request.GET.keys():
        add_filter['domain'] = request.GET['site']
    stories = lambda: list(_newest(page=page,
                                   add_filter=add_filter,
                                   add_q=add_q)
                           )
    stories = cache.get_or_set("news-newest-%s" % (page),
                               stories,
                               timeout=TIMEOUT_SHORT)  # two seconds
    if len(stories) < 1 and page != 0:
        back = _one_page_back(request)
        if back:
            return back
...
def threads(request):
    ...
    tree = Comment.objects.filter(
                        tree_id=OuterRef('tree_id'),
                        user=OuterRef('user')).values('tree_id', 'user__pk')\
                                              .annotate(min_level=Min('level'))
    stories = Comment.objects.filter(user=request.user)\
                             .filter(
        Q(level__in=Subquery(tree.values('min_level'),
          output_field=models.IntegerField()))  # TODO: level= or level__in=???
                                     ).select_related(
                                                'user',
                                                'parent',
                                                'to_story'
...
def comments(request):  # TODO
    ...
    if 'upvoted_by' in request.GET.keys():
        try:
            ...
            return HttpResponseForbidden()
        add_filter['pk__in'] = Vote.objects.filter(vote=1,
                                                   user=request.user
                                                   ).values('item')
    stories = Comment.objects.filter(parent=None).filter(**add_filter)\
                                                 .select_related(
                                                                'user',
                                                                'parent',
                                                                'to_story')\
...
@login_required
@ratelimit(key="user_or_ip", group="news-votes", rate=DEFAULT_VOTES_RATE,
           block=True)
...
def _item_story_comment(pk):
    try:
        ...
        item = Item.objects.select_related('story',
                                           'comment',
                                           'user',
                                           'parent')\
...
@ratelimit(key="user_or_ip", group="news-post", rate=DEFAULT_POST_RATE,
...
def item(request, pk):
    ...
    if request.user.is_authenticated:
        ...
        comment_instance = Comment(user=request.user,
                                   to_story=story,
                                   parent=parent)
        ...
        comment_form = None
    context = {'item': item, 'comment_form': comment_form}
...
def item_edit(request, pk):
    ...
    context = {'item': item, 'edit_form': form}
    return render(request, 'news/item_edit.html', context)
...
def submit(request):
    ...
    context = {'form': form}
    return render(request, 'news/submit.html', context)
